...
1. `workspace/package.json`에 `engines.node`를 `>=26 <27`, `engines.npm`을 `>=11`로 먼저 설정합니다. `dev`는 `node --watch src/server.js`, `lint`는 `eslint "src/**/*.js"`, `format:check`는 `prettier --check . --ignore-unknown`으로 선언합니다.
...
3. `eslint.config.js`는 `@eslint/js`의 `recommended` 설정을 적용하고 `src/**/*.js`에 `ecmaVersion: 'latest'`, `sourceType: 'module'`, 읽기 전용 `console` 전역을 설정합니다. `.prettierrc`는 `{ "printWidth": 80, "bracketSpacing": true, "singleQuote": true, "semi": true, "trailingComma": "all" }`로 완성합니다.
