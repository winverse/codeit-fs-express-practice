...
1. `workspace/src/config/config.js`의 `TODO`를 따라 환경 변수 schema와 변환을 완성합니다.
...
4. `workspace/.gitignore`와 `workspace/env/.env.example`의 `TODO`를 따라 환경 파일 제외 규칙과 예시 값을 완성합니다. `.env.example`은 `NODE_ENV=development`와 `PORT=5001`을 담도록 완성합니다. `dev`나 `start`를 직접 실행할 때는 예시 파일을 바탕으로 실제 `.env.development`와 `.env.production`을 만들되 Git에는 포함하지 않습니다.
