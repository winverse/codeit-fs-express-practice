...
학생이 수정하는 JavaScript 파일에는 `TODO` 주석이 있습니다. 각 문제 README의 파일 순서와 `TODO`를 따라 구현하고, Express 앱을 조립하는 주 파일은 수업과 같은 `src/server.js`를 사용합니다. README가 수정 대상으로 안내하지 않은 파일은 참고용으로만 확인합니다.

각 확인 명령은 처음에는 해당 실습의 TODO 때문에 실패하는 것이 정상입니다. `fixtures/`와 `test/`는 입력·판정 코드로 그대로 사용하고, `answers/`는 직접 구현하고 확인한 뒤에만 비교합니다.
